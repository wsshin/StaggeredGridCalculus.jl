...
function apply_m̂!(Gv::AbsArrNumber{3},  # v-component of output field (v = x, y, z)
    ...
    @sync if isfwd
            ...
            if isbloch
                ...
                for t = 1:Nₜ
                    ...
                    let kₛₜ=kₛₜ, kₑₜ=kₑₜ
                        @spawn for k = kₛₜ:kₑₜ
                            for j = 1:Ny, i = 1:Nx-1
                                @inbounds set_or_add!(Gv, CartesianIndex(i,j,k), α2 * (Fu[i+1,j,k] + Fu[i,j,k]), Val(OP))
                            end
                        end
                    end
                end
                ...
                for k = 1:Nz, j = 1:Ny
                    @inbounds set_or_add!(Gv, CartesianIndex(Nx,j,k), α2 * (e⁻ⁱᵏᴸ*Fu[1,j,k] + Fu[Nx,j,k]), Val(OP))  # Fu[Nx+1,j,k] = exp(-i kx Lx) * Fu[1,j,k]
                end
                ...
                # assumed zero)
                for k = 1:Nz, j = 1:Ny
                    @inbounds set_or_add!(Gv, CartesianIndex(1,j,k), α2 * Fu[2,j,k], Val(OP))  # Fu[1,j,k] == 0
                end

                # 3. At the positive end of the x-direction (where the boundary fields are
                # assumed zero)
                for k = 1:Nz, j = 1:Ny
                    @inbounds set_or_add!(Gv, CartesianIndex(Nx,j,k), α2 * Fu[Nx,j,k], Val(OP))  # Fu[Nx+1,j,k] == 0
                end
            end
        elseif nw == 2  # w = y
            if isbloch
                ...
                for t = 1:Nₜ
                    ...
                    let kₛₜ=kₛₜ, kₑₜ=kₑₜ
                        @spawn for k = kₛₜ:kₑₜ
                            for j = 1:Ny-1, i = 1:Nx
                                @inbounds set_or_add!(Gv, CartesianIndex(i,j,k), α2 * (Fu[i,j+1,k] + Fu[i,j,k]), Val(OP))
                            end
                        end
                    end
                end
                ...
                # taken from the negative-end boundary)
                for k = 1:Nz, i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,Ny,k), α2 * (e⁻ⁱᵏᴸ*Fu[i,1,k] + Fu[i,Ny,k]), Val(OP))  # Fu[i,Ny+1,k] = exp(-i ky Ly) * Fu[i,1,k]
                end
            else  # symmetry boundary
                # 1. At the locations except for the positive and negative ends of the
                # y-direction
                ...
                # assumed zero)
                for k = 1:Nz, i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,1,k), α2 * Fu[i,2,k], Val(OP))  # Fu[i,1,k] == 0
                end

                # 3. At the positive end of the y-direction (where the boundary fields are
                # assumed zero)
                for k = 1:Nz, i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,Ny,k), α2 * Fu[i,Ny,k], Val(OP))  # Fu[i,Ny+1,k] == 0
                end
            end
        else  # nw == 3; w = z
            if isbloch
                ...
                for t = 1:Nₜ
                    ...
                    let kₛₜ=kₛₜ, kₑₜ=kₑₜ
                        @spawn for k = kₛₜ:kₑₜ
                            for j = 1:Ny, i = 1:Nx
                                @inbounds set_or_add!(Gv, CartesianIndex(i,j,k), α2 * (Fu[i,j,k+1] + Fu[i,j,k]), Val(OP))
                            end
                        end
                    end
                end
                ...
                # taken from the negative-end boundary)
                for j = 1:Ny, i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,j,Nz), α2 * (e⁻ⁱᵏᴸ*Fu[i,j,1] + Fu[i,j,Nz]), Val(OP))  # Fu[i,j,Nz+1] = exp(-i kz Lz) * Fu[i,j,1]
                end
            else  # symmetry boundary
                # 1. At the locations except for the positive and negative ends of the
                # z-direction
                ...
                # assumed zero)
                for j = 1:Ny, i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,j,1), α2 * Fu[i,j,2], Val(OP))  # Fu[i,j,1] == 0
                end

                # 3. At the positive end of the z-direction (where the boundary fields are
                # assumed zero)
                for j = 1:Ny, i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,j,Nz), α2 * Fu[i,j,Nz], Val(OP))  # Fu[i,j,Nz+1] == 0
                end
            end
        end  # if nw == ...
        ...
        if nw == 1  # w = x
                ...
                let kₛₜ=kₛₜ, kₑₜ=kₑₜ
                    @spawn for k = kₛₜ:kₑₜ
                        for j = 1:Ny, i = 2:Nx  # not i = 2:Nx-1
                            @inbounds set_or_add!(Gv, CartesianIndex(i,j,k), α2 * (Fu[i,j,k] + Fu[i-1,j,k]), Val(OP))
                        end
                    end
                end
            end
            ...
            if isbloch
                for k = 1:Nz, j = 1:Ny
                    @inbounds set_or_add!(Gv, CartesianIndex(1,j,k), α2 * (Fu[1,j,k] + Fu[Nx,j,k]/e⁻ⁱᵏᴸ), Val(OP))  # Fu[0,j,k] = Fu[Nx,j,k] / exp(-i kx Lx)
                end
            else  # symmetry boundary
                for k = 1:Nz, j = 1:Ny
                    @inbounds set_or_add!(Gv, CartesianIndex(1,j,k), α * Fu[1,j,k], Val(OP))  # Fu[0,j,k] = Fu[1,j,k]; use α instead of α2 as field is doubled
                end
            end
        elseif nw == 2  # w = y
                ...
                let kₛₜ=kₛₜ, kₑₜ=kₑₜ
                    @spawn for k = kₛₜ:kₑₜ
                        for j = 2:Ny, i = 1:Nx  # not j = 2:Ny-1
                            @inbounds set_or_add!(Gv, CartesianIndex(i,j,k), α2 * (Fu[i,j,k] + Fu[i,j-1,k]), Val(OP))
                        end
                    end
                end
            end
            ...
            if isbloch
                for k = 1:Nz, i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,1,k), α2 * (Fu[i,1,k] + Fu[i,Ny,k]/e⁻ⁱᵏᴸ), Val(OP))  # Fu[i,0,k] = Fu[0,Ny,k] / exp(-i ky Ly)
                end
            else  # symmetry boundary
                for k = 1:Nz, i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,1,k), α * Fu[i,1,k], Val(OP))  # Fu[i,0,k] = Fu[i,1,k]; use α instead of α2 as field is doubled
                end
            end
        else  # nw == 3; w = z
            ...
            for t = 1:Nₜ
                ...
                let kₛₜ=kₛₜ, kₑₜ=kₑₜ
                    @spawn for k = kₛₜ:kₑₜ
                        for j = 1:Ny, i = 1:Nx
                            @inbounds set_or_add!(Gv, CartesianIndex(i,j,k), α2 * (Fu[i,j,k] + Fu[i,j,k-1]), Val(OP))
                        end
                    end
                end
            end
            ...
            if isbloch
                for j = 1:Ny, i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,j,1), α2 * (Fu[i,j,1] + Fu[i,j,Nz]/e⁻ⁱᵏᴸ), Val(OP))  # Fu[i,j,0] = Fu[i,j,Nz] / exp(-i kz Lz)
                end
            else  # symmetry boundary
                for j = 1:Ny, i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,j,1), α * Fu[i,j,1], Val(OP))  # Fu[i,j,0] = Fu[i,j,Nz]; use α instead of α2 as field is doubled
                end
            end
        end  # if nw == ...
    end  # if isfwd
...
function apply_m̂!(Gv::AbsArrNumber{2},  # v-component of output field (v = x, y)
    ...
    @sync if isfwd
            ...
            if isbloch
                ...
                for t = 1:Nₜ
                    ...
                    let jₛₜ=jₛₜ, jₑₜ=jₑₜ
                        @spawn for j = jₛₜ:jₑₜ
                            for i = 1:Nx-1
                                @inbounds set_or_add!(Gv, CartesianIndex(i,j), α2 * (Fu[i+1,j] + Fu[i,j]), Val(OP))
                            end
                        end
                    end
                end
                ...
                # taken from the negative-end boundary)
                for j = 1:Ny
                    @inbounds set_or_add!(Gv, CartesianIndex(Nx,j), α2 * (e⁻ⁱᵏᴸ*Fu[1,j] + Fu[Nx,j]), Val(OP))  # Fu[Nx+1,j] = exp(-i kx Lx) * Fu[1,j]
                end
            else  # symmetry boundary
                # 1. At the locations except for the positive and negative ends of the
                # x-direction
                ...
                # assumed zero)
                for j = 1:Ny
                    @inbounds set_or_add!(Gv, CartesianIndex(1,j), α2 * Fu[2,j], Val(OP))  # Fu[1,j] == 0
                end

                # 3. At the positive end of the x-direction (where the boundary fields are
                # assumed zero)
                for j = 1:Ny
                    @inbounds set_or_add!(Gv, CartesianIndex(Nx,j), α2 * Fu[Nx,j], Val(OP))  # Fu[Nx+1,j] == 0
                end
            end
        else  # nw == 2; w = y
            if isbloch
                ...
                for t = 1:Nₜ
                    ...
                    let jₛₜ=jₛₜ, jₑₜ=jₑₜ
                        @spawn for j = jₛₜ:jₑₜ
                            for i = 1:Nx
                                @inbounds set_or_add!(Gv, CartesianIndex(i,j), α2 * (Fu[i,j+1] + Fu[i,j]), Val(OP))
                            end
                        end
                    end
                end
                ...
                # taken from the negative-end boundary)
                for i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,Ny), α2 * (e⁻ⁱᵏᴸ*Fu[i,1] + Fu[i,Ny]), Val(OP))  # Fu[i,Ny+1] = exp(-i ky Ly) * Fu[i,1]
                end
            else  # symmetry boundary
                # 1. At the locations except for the positive and negative ends of the
                # y-direction
                ...
                # assumed zero)
                for i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,1), α2 * Fu[i,2], Val(OP))  # Fu[i,1] == 0
                end

                # 3. At the positive end of the y-direction (where the boundary fields are
                # assumed zero)
                for i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,Ny), α2 * Fu[i,Ny], Val(OP))  # Fu[i,Ny+1] == 0
                end
            end
        end  # if nw == ...
    else  # backward averaging
        if nw == 1  # w = x
                ...
                let jₛₜ=jₛₜ, jₑₜ=jₑₜ
                    @spawn for j = jₛₜ:jₑₜ
                        for i = 2:Nx  # not i = 2:Nx-1
                            @inbounds set_or_add!(Gv, CartesianIndex(i,j), α2 * (Fu[i,j] + Fu[i-1,j]), Val(OP))
                        end
                    end
                end
            end
            ...
            if isbloch
                for j = 1:Ny
                    @inbounds set_or_add!(Gv, CartesianIndex(1,j), α2 * (Fu[1,j] + Fu[Nx,j]/e⁻ⁱᵏᴸ), Val(OP))  # Fu[0,j] = Fu[Nx,j] / exp(-i kx Lx)
                end
            else  # symmetry boundary
                for j = 1:Ny
                    @inbounds set_or_add!(Gv, CartesianIndex(1,j), α * Fu[1,j], Val(OP))  # Fu[0,j] = Fu[1,j]; use α instead of α2 as field is doubled
                end
            end
        elseif nw == 2  # w = y
            # 1. At the locations except for the negative end of the y-direction; unlike for
            ...
            for t = 1:Nₜ
                ...
                let jₛₜ=jₛₜ, jₑₜ=jₑₜ
                    @spawn for j = jₛₜ:jₑₜ
                        for i = 1:Nx
                            @inbounds set_or_add!(Gv, CartesianIndex(i,j), α2 * (Fu[i,j] + Fu[i,j-1]), Val(OP))
                        end
                    end
                end
            end
            ...
            if isbloch
                for i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,1), α2 * (Fu[i,1] + Fu[i,Ny]/e⁻ⁱᵏᴸ), Val(OP))  # Fu[i,0] = Fu[0,Ny] / exp(-i ky Ly)
                end
            else  # symmetry boundary
                for i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,1), α * Fu[i,1], Val(OP))  # Fu[i,0] = Fu[i,1]; use α instead of α2 as field is doubled
                end
            end
        end  # if nw == ...
    end  # if isfwd
    ...
    @sync if isfwd
        if isbloch
            ...
            for t = 1:Nₜ
                ...
                let iₛₜ=iₛₜ, iₑₜ=iₑₜ
                    @spawn for i = iₛₜ:iₑₜ
                        @inbounds set_or_add!(Gv, CartesianIndex(i), α2 * (Fu[i+1] + Fu[i]), Val(OP))
                    end
                end
            end
            ...
            @inbounds set_or_add!(Gv, CartesianIndex(Nx), α2 * (e⁻ⁱᵏᴸ*Fu[1] + Fu[Nx]), Val(OP))  # Fu[Nx+1] = exp(-i kx Lx) * Fu[1]
        else  # symmetry boundary
            ...
            # zero)
            @inbounds set_or_add!(Gv, CartesianIndex(1), α2 * Fu[2], Val(OP))  # Fu[1] == 0

            # 3. At the positive end of the x-direction (where the boundary fields are assumed
            # zero)
            @inbounds set_or_add!(Gv, CartesianIndex(Nx), α2 * Fu[Nx], Val(OP))  # Fu[Nx+1] == 0
        end
    else  # backward averaging
        ...
        for t = 1:Nₜ
            ...
            let iₛₜ=iₛₜ, iₑₜ=iₑₜ
                @spawn for i = iₛₜ:iₑₜ
                    @inbounds set_or_add!(Gv, CartesianIndex(i), α2 * (Fu[i] + Fu[i-1]), Val(OP))
                end
            end
        end
        ...
        # from the positive-end boundary for the Bloch boundary condition)
        if isbloch
            @inbounds set_or_add!(Gv, CartesianIndex(1), α2 * (Fu[1] + Fu[Nx]/e⁻ⁱᵏᴸ), Val(OP))  # Fu[0] = Fu[Nx] / exp(-i kx Lx)
        else  # symmetry boundary
            @inbounds set_or_add!(Gv, CartesianIndex(1), α * Fu[1], Val(OP))  # Fu[0] = Fu[1]; use α instead of α2 as field is doubled
        end
    end  # if isfwd
            ...
            if isbloch
                ...
                for t = 1:Nₜ
                    ...
                    let kₛₜ=kₛₜ, kₑₜ=kₑₜ
                        @spawn for k = kₛₜ:kₑₜ
                            for j = 1:Ny, i = 1:Nx-1
                                @inbounds set_or_add!(Gv, CartesianIndex(i,j,k), (α2 * ∆w′⁻¹[i]) * (∆w[i+1]*Fu[i+1,j,k] + ∆w[i]*Fu[i,j,k]), Val(OP))
                            end
                        end
                    end
                end
                ...
                β = α2 * ∆w′⁻¹[Nx]
                for k = 1:Nz, j = 1:Ny
                    @inbounds set_or_add!(Gv, CartesianIndex(Nx,j,k), β * (∆w[1]*e⁻ⁱᵏᴸ*Fu[1,j,k] + ∆w[Nx]*Fu[Nx,j,k]), Val(OP))  # Fu[Nx+1,j,k] = exp(-i kx Lx) * Fu[1,j,k]
                end
            else  # symmetry boundary
                # 1. At the locations except for the positive and negative ends of the
                # x-direction
                ...
                β = α2 * ∆w′⁻¹[1]
                for k = 1:Nz, j = 1:Ny
                    @inbounds set_or_add!(Gv, CartesianIndex(1,j,k), β * ∆w[2]*Fu[2,j,k], Val(OP))  # Fu[1,j,k] == 0
                end
                ...
                β = α2 * ∆w′⁻¹[Nx]
                for k = 1:Nz, j = 1:Ny
                    @inbounds set_or_add!(Gv, CartesianIndex(Nx,j,k), β * ∆w[Nx]*Fu[Nx,j,k], Val(OP))  # Fu[Nx+1,j,k] == 0
                end
            end
        elseif nw == 2  # w = y
            if isbloch
                ...
                for t = 1:Nₜ
                    ...
                    let kₛₜ=kₛₜ, kₑₜ=kₑₜ
                        @spawn for k = kₛₜ:kₑₜ
                            for j = 1:Ny-1, i = 1:Nx
                                @inbounds set_or_add!(Gv, CartesianIndex(i,j,k), (α2 * ∆w′⁻¹[j]) * (∆w[j+1]*Fu[i,j+1,k] + ∆w[j]*Fu[i,j,k]), Val(OP))
                            end
                        end
                    end
                end
                ...
                β = α2 * ∆w′⁻¹[Ny]
                for k = 1:Nz, i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,Ny,k), β * (∆w[1]*e⁻ⁱᵏᴸ*Fu[i,1,k] + ∆w[Ny]*Fu[i,Ny,k]), Val(OP))  # Fu[i,Ny+1,k] = exp(-i ky Ly) * Fu[i,1,k]
                end
            else  # symmetry boundary
                # 1. At the locations except for the positive and negative ends of the
                # y-direction
                ...
                β = α2 * ∆w′⁻¹[1]
                for k = 1:Nz, i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,1,k), β * ∆w[2]*Fu[i,2,k], Val(OP))  # Fu[i,1,k] == 0
                end
                ...
                β = α2 * ∆w′⁻¹[Ny]
                for k = 1:Nz, i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,Ny,k), β * ∆w[Ny]*Fu[i,Ny,k], Val(OP))  # Fu[i,Ny+1,k] == 0
                end
            end
        else  # nw == 3; w = z
            if isbloch
                ...
                for t = 1:Nₜ
                    ...
                    let kₛₜ=kₛₜ, kₑₜ=kₑₜ
                        @spawn for k = kₛₜ:kₑₜ
                            for j = 1:Ny, i = 1:Nx
                                @inbounds set_or_add!(Gv, CartesianIndex(i,j,k), (α2 * ∆w′⁻¹[k]) * (∆w[k+1]*Fu[i,j,k+1] + ∆w[k]*Fu[i,j,k]), Val(OP))
                            end
                        end
                    end
                end
                ...
                β = α2 * ∆w′⁻¹[Nz]
                for j = 1:Ny, i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,j,Nz), β * (∆w[1]*e⁻ⁱᵏᴸ*Fu[i,j,1] + ∆w[Nz]*Fu[i,j,Nz]), Val(OP))  # Fu[i,j,Nz+1] = exp(-i kz Lz) * Fu[i,j,1]
                end
            else  # symmetry boundary
                # 1. At the locations except for the positive and negative ends of the
                # z-direction
                ...
                β = α2 * ∆w′⁻¹[1]
                for j = 1:Ny, i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,j,1), β * ∆w[2]*Fu[i,j,2], Val(OP))  # Fu[i,j,1] == 0
                end
                ...
                β = α2 * ∆w′⁻¹[Nz]
                for j = 1:Ny, i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,j,Nz), β * ∆w[Nz]*Fu[i,j,Nz], Val(OP))  # Fu[i,j,Nz+1] == 0
                end
            end
        end  # if nw == ...
    else  # backward averaging
        if nw == 1  # w = x
            ...
            for t = 1:Nₜ
                ...
                let kₛₜ=kₛₜ, kₑₜ=kₑₜ
                    @spawn for k = kₛₜ:kₑₜ
                        for j = 1:Ny, i = 2:Nx  # not i = 2:Nx-1
                            @inbounds set_or_add!(Gv, CartesianIndex(i,j,k), (α2 * ∆w′⁻¹[i]) * (∆w[i]*Fu[i,j,k] + ∆w[i-1]*Fu[i-1,j,k]), Val(OP))
                        end
                    end
                end
            end
            ...
            if isbloch
                β = α2 * ∆w′⁻¹[1]
                for k = 1:Nz, j = 1:Ny
                    @inbounds set_or_add!(Gv, CartesianIndex(1,j,k), β * (∆w[1]*Fu[1,j,k] + ∆w[Nx]*Fu[Nx,j,k]/e⁻ⁱᵏᴸ), Val(OP))  # Fu[0,j,k] = Fu[Nx,j,k] / exp(-i kx Lx)
                end
            else  # symmetry boundary
                β = α * ∆w′⁻¹[1]
                for k = 1:Nz, j = 1:Ny
                    @inbounds set_or_add!(Gv, CartesianIndex(1,j,k), β * ∆w[1]*Fu[1,j,k], Val(OP))  # Fu[0,j,k] = Fu[1,j,k]
                end
            end
        elseif nw == 2  # w = y
            # 1. At the locations except for the negative end of the y-direction; unlike for
            ...
            for t = 1:Nₜ
                ...
                let kₛₜ=kₛₜ, kₑₜ=kₑₜ
                    @spawn for k = kₛₜ:kₑₜ
                        for j = 2:Ny, i = 1:Nx  # not j = 2:Ny-1
                            @inbounds set_or_add!(Gv, CartesianIndex(i,j,k), (α2 * ∆w′⁻¹[j]) * (∆w[j]*Fu[i,j,k] + ∆w[j-1]*Fu[i,j-1,k]), Val(OP))
                        end
                    end
                end
            end
            ...
            if isbloch
                β = α2 * ∆w′⁻¹[1]
                for k = 1:Nz, i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,1,k), β * (∆w[1]*Fu[i,1,k] + ∆w[Ny]*Fu[i,Ny,k]/e⁻ⁱᵏᴸ), Val(OP))  # Fu[i,0,k] = Fu[0,Ny,k] / exp(-i ky Ly)
                end
            else  # symmetry boundary
                β = α * ∆w′⁻¹[1]
                for k = 1:Nz, i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,1,k), β * ∆w[1]*Fu[i,1,k], Val(OP))  # Fu[i,0,k] = Fu[i,1,k]
                end
            end
        else  # nw == 3; w = z
            # 1. At the locations except for the negative end of the z-direction; unlike for
            ...
            for t = 1:Nₜ
                ...
                let kₛₜ=kₛₜ, kₑₜ=kₑₜ
                    @spawn for k = kₛₜ:kₑₜ
                        for j = 1:Ny, i = 1:Nx
                            @inbounds set_or_add!(Gv, CartesianIndex(i,j,k), (α2 * ∆w′⁻¹[k]) * (∆w[k]*Fu[i,j,k] + ∆w[k-1]*Fu[i,j,k-1]), Val(OP))
                        end
                    end
                end
            end
            ...
            if isbloch
                β = α2 * ∆w′⁻¹[1]
                for j = 1:Ny, i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,j,1), β * (∆w[1]*Fu[i,j,1] + ∆w[Nz]*Fu[i,j,Nz]/e⁻ⁱᵏᴸ), Val(OP))  # Fu[i,j,0] = Fu[i,j,Nz] / exp(-i kz Lz)
                end
            else  # symmetry boundary
                β = α * ∆w′⁻¹[1]
                for j = 1:Ny, i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,j,1), β * ∆w[1]*Fu[i,j,1], Val(OP))  # Fu[i,j,0] = Fu[i,j,Nz]
                end
            end
        end  # if nw == ...
    end  # if isfwd
            ...
            if isbloch
                ...
                for t = 1:Nₜ
                    ...
                    let jₛₜ=jₛₜ, jₑₜ=jₑₜ
                        @spawn for j = jₛₜ:jₑₜ
                            for i = 1:Nx-1
                                @inbounds set_or_add!(Gv, CartesianIndex(i,j), (α2 * ∆w′⁻¹[i]) * (∆w[i+1]*Fu[i+1,j] + ∆w[i]*Fu[i,j]), Val(OP))
                            end
                        end
                    end
                end
                ...
                β = α2 * ∆w′⁻¹[Nx]
                for j = 1:Ny
                    @inbounds set_or_add!(Gv, CartesianIndex(Nx,j), β * (∆w[1]*e⁻ⁱᵏᴸ*Fu[1,j] + ∆w[Nx]*Fu[Nx,j]), Val(OP))  # Fu[Nx+1,j] = exp(-i kx Lx) * Fu[1,j]
                end
            else  # symmetry boundary
                # 1. At the locations except for the positive and negative ends of the
                # x-direction
                ...
                β = α2 * ∆w′⁻¹[1]
                for j = 1:Ny
                    @inbounds set_or_add!(Gv, CartesianIndex(1,j), β * ∆w[2]*Fu[2,j], Val(OP))  # Fu[1,j] == 0
                end
                ...
                β = α2 * ∆w′⁻¹[Nx]
                for j = 1:Ny
                    @inbounds set_or_add!(Gv, CartesianIndex(Nx,j), β * ∆w[Nx]*Fu[Nx,j], Val(OP))  # Fu[Nx+1,j] == 0
                end
            end
        else  # nw == 2; w = y
            if isbloch
                ...
                for t = 1:Nₜ
                    ...
                    let jₛₜ=jₛₜ, jₑₜ=jₑₜ
                        @spawn for j = jₛₜ:jₑₜ
                            for i = 1:Nx
                                @inbounds set_or_add!(Gv, CartesianIndex(i,j), (α2 * ∆w′⁻¹[j]) * (∆w[j+1]*Fu[i,j+1] + ∆w[j]*Fu[i,j]), Val(OP))
                            end
                        end
                    end
                end
                ...
                β = α2 * ∆w′⁻¹[Ny]
                for i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,Ny), β * (∆w[1]*e⁻ⁱᵏᴸ*Fu[i,1] + ∆w[Ny]*Fu[i,Ny]), Val(OP))  # Fu[i,Ny+1] = exp(-i ky Ly) * Fu[i,1]
                end
            else  # symmetry boundary
                # 1. At the locations except for the positive and negative ends of the
                # y-direction
                ...
                β = α2 * ∆w′⁻¹[1]
                for i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,1), β * ∆w[2]*Fu[i,2], Val(OP))  # Fu[i,1] == 0
                end
                ...
                β = α2 * ∆w′⁻¹[Ny]
                for i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,Ny), β * ∆w[Ny]*Fu[i,Ny], Val(OP))  # Fu[i,Ny+1] == 0
                end
            end
        end  # if nw == ...
    else  # backward averaging
        if nw == 1  # w = x
            ...
            for t = 1:Nₜ
                ...
                let jₛₜ=jₛₜ, jₑₜ=jₑₜ
                    @spawn for j = jₛₜ:jₑₜ
                        for i = 2:Nx  # not i = 2:Nx-1
                            @inbounds set_or_add!(Gv, CartesianIndex(i,j), (α2 * ∆w′⁻¹[i]) * (∆w[i]*Fu[i,j] + ∆w[i-1]*Fu[i-1,j]), Val(OP))
                        end
                    end
                end
            end
            ...
            if isbloch
                β = α2 * ∆w′⁻¹[1]
                for j = 1:Ny
                    @inbounds set_or_add!(Gv, CartesianIndex(1,j), β * (∆w[1]*Fu[1,j] + ∆w[Nx]*Fu[Nx,j]/e⁻ⁱᵏᴸ), Val(OP))  # Fu[0,j] = Fu[Nx,j] / exp(-i kx Lx)
                end
            else  # symmetry boundary
                β = α * ∆w′⁻¹[1]
                for j = 1:Ny
                    @inbounds set_or_add!(Gv, CartesianIndex(1,j), β * ∆w[1]*Fu[1,j], Val(OP))  # Fu[0,j] = Fu[1,j]
                end
            end
        elseif nw == 2  # w = y
            # 1. At the locations except for the negative end of the y-direction; unlike for
            ...
            for t = 1:Nₜ
                ...
                let jₛₜ=jₛₜ, jₑₜ=jₑₜ
                    @spawn for j = jₛₜ:jₑₜ
                        for i = 1:Nx
                            @inbounds set_or_add!(Gv, CartesianIndex(i,j), (α2 * ∆w′⁻¹[j]) * (∆w[j]*Fu[i,j] + ∆w[j-1]*Fu[i,j-1]), Val(OP))
                        end
                    end
                end
            end
            ...
            if isbloch
                β = α2 * ∆w′⁻¹[1]
                for i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,1), β * (∆w[1]*Fu[i,1] + ∆w[Ny]*Fu[i,Ny]/e⁻ⁱᵏᴸ), Val(OP))  # Fu[i,0] = Fu[0,Ny] / exp(-i ky Ly)
                end
            else  # symmetry boundary
                β = α * ∆w′⁻¹[1]
                for i = 1:Nx
                    @inbounds set_or_add!(Gv, CartesianIndex(i,1), β * ∆w[1]*Fu[i,1], Val(OP))  # Fu[i,0] = Fu[i,1]
                end
            end
        end  # if nw == ...
    end  # if isfwd
    ...
    @sync if isfwd
        if isbloch
            ...
            for t = 1:Nₜ
                ...
                let iₛₜ=iₛₜ, iₑₜ=iₑₜ
                    @spawn for i = iₛₜ:iₑₜ
                        @inbounds set_or_add!(Gv, CartesianIndex(i), (α2 * ∆w′⁻¹[i]) * (∆w[i+1]*Fu[i+1] + ∆w[i]*Fu[i]), Val(OP))
                    end
                end
            end
            ...
            # from the negative-end boundary)
            β = α2 * ∆w′⁻¹[Nx]
            @inbounds set_or_add!(Gv, CartesianIndex(Nx), β * (∆w[1]*e⁻ⁱᵏᴸ*Fu[1] + ∆w[Nx]*Fu[Nx]), Val(OP))  # Fu[Nx+1] = exp(-i kx Lx) * Fu[1]
        else  # symmetry boundary
            # 1. At the locations except for the positive and negative ends of the x-direction
            iₛ = iₛ + ∆iₛ  # initially iₛ[1] = 1; now iₛ[1] = 2
            iₑ = iₑ - ∆iₑ  # initially iₑ[Nₜ] = Nx; now iₑ[Nₜ] = Nx-1
            ...
            # zero)
            β = α2 * ∆w′⁻¹[1]
            @inbounds set_or_add!(Gv, CartesianIndex(1), β * ∆w[2]*Fu[2], Val(OP))  # Fu[1] == 0

            # 3. At the positive end of the x-direction (where the boundary fields are assumed
            # zero)
            β = α2 * ∆w′⁻¹[Nx]
            @inbounds set_or_add!(Gv, CartesianIndex(Nx), β * ∆w[Nx]*Fu[Nx], Val(OP))  # Fu[Nx+1] == 0
        end
    else  # backward averaging
        # 1. At the locations except for the negative end of the x-direction; unlike for the
        # forward difference, for the backward difference this part of the code is common
        ...
        for t = 1:Nₜ
            ...
            let iₛₜ=iₛₜ, iₑₜ=iₑₜ
                @spawn for i = iₛₜ:iₑₜ
                    @inbounds set_or_add!(Gv, CartesianIndex(i), (α2 * ∆w′⁻¹[i]) * (∆w[i]*Fu[i] + ∆w[i-1]*Fu[i-1]), Val(OP))
                end
            end
        end
        ...
        if isbloch
            β = α2 * ∆w′⁻¹[1]
            @inbounds set_or_add!(Gv, CartesianIndex(1), β * (∆w[1]*Fu[1] + ∆w[Nx]*Fu[Nx]/e⁻ⁱᵏᴸ), Val(OP))  # Fu[0] = Fu[Nx] / exp(-i kx Lx)
        else  # symmetry boundary
            β = α * ∆w′⁻¹[1]
            @inbounds set_or_add!(Gv, CartesianIndex(1), β * ∆w[1]*Fu[1], Val(OP))  # Fu[0] = Fu[1]
        end
    end  # if isfwd
